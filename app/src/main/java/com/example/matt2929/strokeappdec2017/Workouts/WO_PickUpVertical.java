...
public class WO_PickUpVertical extends SensorWorkoutAbstract {
	...
	public WO_PickUpVertical(String Name, Integer reps, SpeechTrigger speechTrigger, EndRepTrigger endRepTrigger, SFXPlayer sfxPlayer, OutputWorkoutData outputWorkoutData, OutputWorkoutStrings outputWorkoutStrings) {
			...
			if (AverageDataValue[sensorChoice] < thresehold) {
				...
				if (belowThresholdCount > belowThresholdMax) {
					if (moving == true) {
						...
						pickUpCount++;
						outputWorkoutStrings.getStrings(new String[]{pickUpCount + ""});
